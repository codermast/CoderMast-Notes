...
在 Windows 系统中的文件层级分割符是 `\`，而在 Linux 系统中是 `/`，Java 是一个跨平台语言，如果我们直接将路径名写成绝对的，那么在 Windows 上能运行的程序，可能在 Linux 系统内无法正常运行。

故为了跨平台的兼容性，一般都使用提供的构造方法来进行拼接。

Java 中也提供了 `File.separator` 属性来根据当前运行的操作系统，获取文件分隔符。
